...
#include<string.h>
int main(){
	int ar[26],br[26],I,len,brk,J,t,step;
	char str[10000];
	scanf("%d",&t);
	while(t--){
	...
	if(len%2!=0) printf("-1\n");
		...
		for(I=0;I<len/2;I++) ar[str[I]-'a']++;
		for(I=len/2;I<len;I++) br[str[I]-'a']++;
		brk=0;
		for(I=0;I<26 && brk<len;I++){
			brk=brk+ar[I]+br[I];
			if(ar[I]==br[I]){
				ar[I]=br[I]=0;
			}
...
/*
d is obsessed with reading short stories. Being a CS student, he is doing some interesting frequency analysis with the books. He chooses strings S1 and S2 in such a way that |len(S1)-len(S2)|=1.
